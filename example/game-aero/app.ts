...
Drone.prototype.tick = function (t) {
  ...
  if (this.accCX !== null && this.accCY !== null) {
    const p = this.x - this.accCX;
    const q = this.y - this.accCY;
    const inn = p * this.vx + q * this.vy;
    const out = p * this.vy - q * this.vx;
    const b = (out * 2) / t;
    const v2 = this.v * this.v;
    let d = b * b - 4 * v2 * (v2 + (inn * 2) / t);
    if (d >= 0) {
      d = math.sqrt(d);
      const m1 = (((b - d) / 2 / v2) * this.v) / t;
      const m2 = (((-b - d) / 2 / v2) * this.v) / t;
...
const drone = new Drone(speed, speed * 2, acc);
world.add(drone);
...
const tilt = {
  ...
  update: function (a, b, g, o) {
    const now = Date.now();
    if (!this.watching || now - this.time < 300) {
      return;
    }
    if (this.time === 0) {
    ...
    const x = math.wrap(this.g - this.g0, -180, 180) / 180;
    const y = math.wrap(this.b - this.b0, -180, 180) / 180;
    const min = 0.05;
