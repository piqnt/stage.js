...
Image.prototype.image = function(image) {
  ...
  this._textures[0] = this._image.clone();
  this._textures.length = 1;
  return this;
};

Image.prototype.cropX = function(x, w) {
...
Image.prototype.cropY = function(h, y) {
...
Image.prototype.tile = function(inner) {
  this._tile(false, inner);
  return this;
};

Image.prototype.stretch = function(inner) {
  this._tile(true, inner);
  return this;
};

Image.prototype._tile = function(stretch, inner) {
  var self = this;
  this.untick(this._tileTicker);
  this.tick(this._tileTicker = function() {
    ...
    this._textures.length = tile(this._image, width, height, stretch, inner,
        insert);
  });

  function insert(i, sx, sy, sw, sh, dx, dy, dw, dh) {
    var tile = self._textures.length > i ? self._textures[i]
        : self._textures[i] = self._image.clone();
    tile.src(sx, sy, sw, sh);
    tile.dest(dx, dy, dw, dh);
  }
};
...
Anim.prototype.frames = function(frames) {
  ...
  this._frames = [];
  this._labels = {};
  frames = Cut.texture(frames, true);
  if (frames && frames.length) {
    for (var i = 0; i < frames.length; i++) {
      var frame = frames[i];
      this._frames.push(frame);
      if (frame.name) {
        this._labels[frame.name] = i;
      }
    }
  }
  this.touch();
...
Anim.prototype.gotoFrame = function(frame, resize) {
  frame = math.rotate(frame, this._frames.length) | 0;
  this._index = frame;
...
Anim.prototype.gotoLabel = function(label, resize) {
...
Str.prototype.value = function(value) {
  ...
  for (var i = 0; i < value.length; i++) {
    var image = Cut.texture(this._frames(value[i]));
    var tile = this._textures.length > i ? this._textures[i]
        : this._textures[i] = image.clone();
    width += i > 0 ? this._spacing : 0;
    tile.src(image.width, image.height);
    tile.dest(width, 0);
