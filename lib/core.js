...
Cut.Image.prototype.draw = function(context) {
  if (this._image !== null) {
    if (this._repeats !== null) {
      var repeats = this._repeats, slice = this._image;
      var sx, sy, sw, sh, dx, dy, dw, dh;
      for (var i = 0, n = repeats.length; i < n; i += 8) {
        sx = repeats[i + 0], sy = repeats[i + 1];
        sw = repeats[i + 2], sh = repeats[i + 3];
        dx = repeats[i + 4], dy = repeats[i + 5];
        dw = repeats[i + 6] || sw, dh = repeats[i + 7] || sh;
        slice.sX(sx).sY(sy).sWidth(sw).sHeight(sh);
        slice.dX(dx).dY(dy).dWidth(dw).dHeight(dh);
        slice.draw(context);
      }
    } else {
      this._image.draw(context);
    }
  }
};
...
Cut.Image.prototype.image = function(cutout) {
  this._image = Cut.cutout(cutout);
  this.pin('width', this._image ? this._image.dWidth() : 0);
  this.pin('height', this._image ? this._image.dHeight() : 0);
  if (this._repeats !== null) {
    this._repeats.length = 0;
  }
  return this;
};

Cut.Image.prototype.cropX = function(w, x) {
  this._image.sWidth(w);
  this._image.sX(x);
  this.pin('width', this._image.dWidth());
  return this;
};

Cut.Image.prototype.cropY = function(h, y) {
  this._image.sHeight(h);
  this._image.sY(y);
  this.pin('height', this._image.dHeight());
  return this;
};
...
Cut.Image.prototype._repeat = function(inner, stretch) {
  ...
  this.tick(this._repeatTicker = function() {
    ...
    var ileft = this._image._left;
    var iright = this._image._right;
    var itop = this._image._top;
    var ibottom = this._image._bottom;
    var iwidth = this._image._width - ileft - iright;
    var iheight = this._image._height - itop - ibottom;
    var owidth = this.pin('width');
    var oheight = this.pin('height');
    owidth = inner ? owidth : Math.max(owidth - ileft - iright, 0);
    oheight = inner ? oheight : Math.max(oheight - itop - ibottom, 0);

    this._repeats.length = 8 * Cut.Image._nine(ileft, iright, itop, ibottom,
        iwidth, iheight, owidth, oheight, stretch, insert);
  });

  function insert(i, sx, sy, sw, sh, dx, dy, dw, dh) {
    self._repeats[i * 8 + 0] = sx;
    self._repeats[i * 8 + 1] = sy;
    self._repeats[i * 8 + 2] = sw;
    self._repeats[i * 8 + 3] = sh;
    self._repeats[i * 8 + 4] = dx;
    self._repeats[i * 8 + 5] = dy;
    self._repeats[i * 8 + 6] = dw || sw;
    self._repeats[i * 8 + 7] = dh || sh;
  }
};

Cut.Image._nine = function(ileft, iright, itop, ibottom, iwidth, iheight,
    owidth, oheight, stretch, insert) {

  var i = 0;

  // top, left
  if (itop && ileft) {
    insert(i++, 0, 0, ileft, itop, 0, 0);
  }

  // bottom, left
  if (ibottom && ileft) {
    insert(i++, 0, iheight + itop, ileft, ibottom, 0, oheight + itop);
  }

  // top, right
  if (itop && iright) {
    insert(i++, iwidth + ileft, 0, iright, itop, owidth + ileft, 0);
  }

  // bottom, right
  if (ibottom && iright) {
    insert(i++, iwidth + ileft, iheight + itop, iright, ibottom,
        owidth + ileft, oheight + itop);
  }

  if (stretch) {
    // top
    if (itop) {
      insert(i++, ileft, 0, iwidth, itop, ileft, 0, owidth, 0);
    }
    // bottom
    if (ibottom) {
      insert(i++, ileft, iheight + itop, iwidth, ibottom, ileft,
          oheight + itop, owidth, 0);
    }
    // left
    if (ileft) {
      insert(i++, 0, itop, ileft, iheight, 0, itop, 0, oheight);
    }
    // right
    if (iright) {
      insert(i++, iwidth + ileft, itop, iright, iheight, owidth + ileft, itop,
          0, oheight);
    }
    // center
    insert(i++, ileft, itop, iwidth, iheight, ileft, itop, owidth, oheight);

  } else { // tile

    var left = ileft, right = owidth;
    while (right > 0) {
      var width = Math.min(iwidth, right);
      right -= iwidth;

      var top = itop, bottom = oheight;
      while (bottom > 0) {
        var height = Math.min(iheight, bottom);
        bottom -= iheight;

        insert(i++, ileft, itop, width, height, left, top);
        if (right <= 0) {
          // left
          if (ileft) {
            insert(i++, 0, itop, ileft, height, 0, top);
          }
          // right
          if (iright) {
            insert(i++, iwidth + ileft, itop, iright, height, left + width, top);
          }
        }
        top += height;
      }
      // top
      if (itop) {
        insert(i++, ileft, 0, width, itop, left, 0);
      }
      // bottom
      if (ibottom) {
        insert(i++, ileft, iheight + itop, width, ibottom, left, top);
      }
      left += width;
    }
  }
...
Cut.Anim.prototype.draw = function(context) {
  this._currentFrame.draw(context);
};
...
Cut.Anim.prototype.gotoFrame = function(frame, resize) {
  ...
  this._index = frame;
  resize = resize || !this._currentFrame;
  this._currentFrame = this._frames[this._index];
  if (resize) {
    this.pin('width', this._currentFrame.dWidth());
    this.pin('height', this._currentFrame.dHeight());
  }
...
Cut.Root = function(request, render) {
  ...
  var loop = function(now) {
    ...
    Cut._stats.fps = elapsed ? 1000 / elapsed : 0;
  };
...
Cut.Out.prototype.draw = function(context) {
  Cut._stats.cutout++;
  ...
  try {
    ...
    if (!this._failed) {
      console.log('Unable to draw cutout: ' + this + ' ' + this._image);
      this._failed = true;
    }
  }
};
