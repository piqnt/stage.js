...
Cut.Image.prototype.image = function(cutout) {
  this._images[0] = Cut.cutout(cutout);
  this._images.length = 1;
  this.pin('width', this._images[0] ? this._images[0].dWidth() : 0);
  this.pin('height', this._images[0] ? this._images[0].dHeight() : 0);
  this._image = this._images[0].clone();
  return this;
};

Cut.Image.prototype.cropX = function(w, x) {
  this._images[0].sWidth(w);
  this._images[0].sX(x);
  return this.image(this._images[0]);
};

Cut.Image.prototype.cropY = function(h, y) {
  this._images[0].sHeight(h);
  this._images[0].sY(y);
  return this.image(this._images[0]);
};
...
Cut.Image.prototype._slice = function(i) {
  return this._images[i] || (this._images[i] = this._image.clone());
};
...
Cut.Image.prototype._repeat = function(inner, stretch) {
  ...
  var cleft = this._image._left;
  var cright = this._image._right;
  var ctop = this._image._top;
  var cbottom = this._image._bottom;

  var cwidth = this._image._width - cleft - cright;
  var cheight = this._image._height - ctop - cbottom;
