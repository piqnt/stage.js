# API Reference

## Classes

- [Anim](classes/Anim)
- [Atlas](classes/Atlas)
- [CanvasTexture](classes/CanvasTexture)
- [ImageTexture](classes/ImageTexture)
- [Matrix](classes/Matrix)
- [Monotype](classes/Monotype)
- [Node](classes/Node)
- [Pin](classes/Pin)
- [PipeTexture](classes/PipeTexture)
- [ResizableTexture](classes/ResizableTexture)
- [Root](classes/Root)
- [Sprite](classes/Sprite)
- [Texture](classes/Texture)
- [TextureSelection](classes/TextureSelection)
- [Transition](classes/Transition)

## Interfaces

- [AtlasDefinition](interfaces/AtlasDefinition)
- [AtlasTextureDefinition](interfaces/AtlasTextureDefinition)
- [MatrixValue](interfaces/MatrixValue)
- [Vec2Value](interfaces/Vec2Value)

## Type Aliases

- [FitMode](type-aliases/FitMode)
- [LegacyFitMode](type-aliases/LegacyFitMode)
- [NodeEventListener](type-aliases/NodeEventListener)
- [NodeTickListener](type-aliases/NodeTickListener)
- [ResizableTextureMode](type-aliases/ResizableTextureMode)
- [TextureSelectionInput](type-aliases/TextureSelectionInput)
- [TextureSelectionInputArray](type-aliases/TextureSelectionInputArray)
- [TextureSelectionInputFactory](type-aliases/TextureSelectionInputFactory)
- [TextureSelectionInputMap](type-aliases/TextureSelectionInputMap)
- [TextureSelectionInputOne](type-aliases/TextureSelectionInputOne)
- [TransitionEndListener](type-aliases/TransitionEndListener)
- [TransitionOptions](type-aliases/TransitionOptions)
- [Viewbox](type-aliases/Viewbox)
- [Viewport](type-aliases/Viewport)

## Variables

- [math](variables/math)
- [POINTER\_CANCEL](variables/POINTER_CANCEL)
- [POINTER\_CLICK](variables/POINTER_CLICK)
- [POINTER\_END](variables/POINTER_END)
- [POINTER\_MOVE](variables/POINTER_MOVE)
- [POINTER\_START](variables/POINTER_START)

## Functions

- [anim](functions/anim)
- [atlas](functions/atlas)
- [box](functions/box)
- [canvas](functions/canvas)
- [column](functions/column)
- [create](functions/create)
- [layer](functions/layer)
- [layout](functions/layout)
- [maximize](functions/maximize)
- [minimize](functions/minimize)
- [monotype](functions/monotype)
- [mount](functions/mount)
- [pause](functions/pause)
- [resume](functions/resume)
- [row](functions/row)
- [sprite](functions/sprite)
- [texture](functions/texture)
