...
function init() {
  ...
  var path = [ [ 0, 0.3 ], [ 0.2, -0.2 ], [ 0, -0.1 ], [ -0.2, -0.2 ] ];
...
function tick() {
  ...
  if (key.shoot && !hideShip && world.time > lastShootTime + shipReloadTime) {
    ...
    bulletBody.velocity[1] += magnitude * Math.sin(angle)
        + shipBody.velocity[1];
    bulletBody.position[0] = shipSize * Math.cos(angle) + shipBody.position[0];
    bulletBody.position[1] = shipSize * Math.sin(angle) + shipBody.position[1];
